...
namespace iAssist.Controllers
{
    ...
    public class ComplainController : Controller
    {
        ...
        public ActionResult ReportWorkerOnTask(int? id)
        {
            var user = User.Identity.GetUserId();
            var worker = db.RegistWork.Where(x => x.Id == id).FirstOrDefault();
            if (worker != null && worker.Userid == user)
        ...
        [HttpPost]
        public ActionResult ReportWorkerOnTask(ComplainViews model)
        {
            if (ModelState.IsValid)
            {
                var complaints = new Complaint();
                ...
                db.Notifications.Add(notification);
                db.SaveChanges();
                return RedirectToAction("ShowMyTaskPost", "Task", new { id = model.Workerid });
            }
            return View(model);
        }
